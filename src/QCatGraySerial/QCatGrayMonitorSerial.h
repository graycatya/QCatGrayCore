...
class QCatGrayMonitorSerial : public QThread
{
    Q_OBJECT
public:
    static QCatGrayMonitorSerial* Instance() noexcept
    {
        if(_instance == nullptr)
        {
            ...
            if(_instance == nullptr)
            {
                _instance = new QCatGrayMonitorSerial;
            }
            m_pMutex->unlock();
        }
        return _instance;
...
private:
    QCatGrayMonitorSerial() {
        m_lSerialPortInfo.clear();
    }
    ~QCatGrayMonitorSerial() {}

    static QCatGrayMonitorSerial* _instance;
