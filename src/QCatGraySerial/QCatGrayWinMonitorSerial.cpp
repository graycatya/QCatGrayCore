...
#include "QCatGrayMonitorSerial.h"
...
QCatGrayWinMonitorSerial* QCatGrayWinMonitorSerial::_instance = nullptr;
QMutex* QCatGrayWinMonitorSerial::m_pMutex = new QMutex;
...
void UpdateDevice(PDEV_BROADCAST_DEVICEINTERFACE pDevInf, WPARAM wParam)
{
    ...
    if (DBT_DEVICEARRIVAL == wParam)
    {
        ...
        if(mes.at(0) == "USB")
        {
            ...
            if(!QCatGrayWinMonitorSerial::Instance()->m_pSerials.contains(mes.at(1)))
            {
                QCatGrayWinMonitorSerial::Instance()->m_pSerials[mes.at(1)] = WDK_WhoAllVidPid( mes.at(1).toStdString() );
                emit QCatGrayWinMonitorSerial::Instance()->AddSerial(npid, nvid, WDK_WhoAllVidPid( mes.at(1).toStdString() ));
            }
            ;
        }
    ...
    else
    {
        ...
        if(mes.at(0) == "USB")
        {
            ...
            if(QCatGrayWinMonitorSerial::Instance()->m_pSerials.contains(mes.at(1)))
            {
                QString dev = QCatGrayWinMonitorSerial::Instance()->m_pSerials[mes.at(1)];
                QCatGrayWinMonitorSerial::Instance()->m_pSerials.remove(mes.at(1));
                emit QCatGrayWinMonitorSerial::Instance()->DeleteSerial(npid, nvid, dev);
            }
        }
    }
...
QStringList QCatGrayWinMonitorSerial::Serials()
{
    return getAvailableSerialPort();
}

QCatGrayWinMonitorSerial::QCatGrayWinMonitorSerial()
{
    m_pSerials.clear();
    hThread = CreateThread(NULL, 0, ThrdFunc, NULL, 0, &iThread);
    if (hThread == NULL) {
        //QString log = "QCatGrayWinMonitorSerial CreateThread Error ";
